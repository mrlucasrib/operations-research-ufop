...
param qtd_cmc, integer, >= 0;
param qtd_clientes, integer, >= 0;

# Conjuntos
set I := 1..qtd_cmc; # CMCs
...
var x_transportation_aoc{Ki};
var z_binary, binary #todo:
var y_vehicle_loads_when_arrives_at_node{(k,j) in Ki, Ni} # todo: erro
...
subject to if_customer_is_covered_by_CMC_then_at_least_one_vehicle_should_visit_the_customer_c9 {i in I, k in Ki, u in J}:
    sum{j in Ni diff  ZEROi} z_binary[j,u,k] <= M_large_positive_number * v_binary[i, u];

# Constraint (10)
subject to guarantee_that_any_vehicle_starts_from_0i_and_finally_visits_Ti_c10 {i in I, k in Ki}:
    sum{j in Ni diff ZEROi} z_binary[ZEROi, j, k] = 1; #TODO: Possivel erro em usar ZEROi como index

# Constraint (11)
subject to guarantee_that_any_vehicle_starts_from_0i_and_finally_visits_Ti_c11 {i in I, k in Ki}:
    sum{j in Ni diff ZEROi} z_binary[j,Ti, k] = 1; #TODO: Possivel erro em usar Ti como index

# Constraint (12)
subject to eliminate_subtours_c12 {i in I, j in Ni diff Ti, jj in Ni diff (J union ZEROi), k in Ki}: #TODO: Possivel erro em union
